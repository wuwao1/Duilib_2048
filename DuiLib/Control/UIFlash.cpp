...
std::shared_ptr<CControlUI> CFlashUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_FLASH) return std::dynamic_pointer_cast<CFlashUI>(shared_from_this());
...
bool CFlashUI::DoCreateControl()
{
    ...
    GetControl(__uuidof(IShockwaveFlash), (void**) &m_pFlash);
...
LRESULT CFlashUI::TranslateAccelerator(MSG *pMsg)
{
    ...
    if(FAILED(m_pFlash->QueryInterface(IID_IOleInPlaceActiveObject, (void* *) &pObj)))
        return S_FALSE;
