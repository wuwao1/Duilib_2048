...
std::shared_ptr<CControlUI> CSliderUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_SLIDER) return std::dynamic_pointer_cast<CSliderUI>(shared_from_this());
...
void CSliderUI::DoEvent(TEventUI& event)
{
    ...
    if(event.Type == UIEVENT_BUTTONUP || event.Type == UIEVENT_RBUTTONUP)
    {
        if(IsEnabled())
        {
            ...
            {
                m_nValue = nValue;
                m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_VALUECHANGED);
                Invalidate();
            }
            UpdateText();
            return;
    ...
    if(event.Type == UIEVENT_MOUSEMOVE)
    {
        if((m_uButtonState & UISTATE_CAPTURED) != 0)
        {
            ...
            {
                UpdateText();
                m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_VALUECHANGED_MOVE);
            }
            Invalidate();
        }
