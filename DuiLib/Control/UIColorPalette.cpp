...
static DWORD HSLToRGB(float H, float S, float L)
{
    ...
    {
        //HSL from 0 to 1
        R = G = B = (unsigned char)(L * 255.0f);    //RGB results from 0 to 255
    }
    else
    {
        ...
        R = (unsigned char)(255.0f * HueToRGB(var_1, var_2, H + (1.0f / 3.0f)));
        G = (unsigned char)(255.0f * HueToRGB(var_1, var_2, H));
        B = (unsigned char)(255.0f * HueToRGB(var_1, var_2, H - (1.0f / 3.0f)));
    }
...
std::shared_ptr<CControlUI> CColorPaletteUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_COLORPALETTE) return std::dynamic_pointer_cast<CColorPaletteUI>(shared_from_this());
...
void CColorPaletteUI::DoInit()
{
    ...
    m_pBits = (unsigned char *) malloc(dwSize);
...
void CColorPaletteUI::DoEvent(TEventUI& event)
{
    ...
    if(event.Type == UIEVENT_BUTTONUP)
    {
        ...
        {
            m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_COLORCHANGED, GetSelectColor(), 0);
        }
...
void CColorPaletteUI::UpdatePalletData()
{
    for(int y = 0; y < 200; ++y)
    {
        for(int x = 0; x < 360; ++x)
        {
            unsigned char *pPiexl = LPBYTE(m_pBits) + ((200 - y)*m_bmInfo.bmWidthBytes) + ((x*m_bmInfo.bmBitsPixel) / 8);
...
void CColorPaletteUI::UpdateBarData()
{
    ...
    for(int y = 0; y < m_nBarHeight; ++y)
    {
        for(int x = 0; x < 200; ++x)
        {
            unsigned char *pPiexl = LPBYTE(m_pBits) + ((210 + y)*m_bmInfo.bmWidthBytes) + ((x*m_bmInfo.bmBitsPixel) / 8);
