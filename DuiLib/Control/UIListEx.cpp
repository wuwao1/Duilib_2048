...
std::shared_ptr<CControlUI> CListExUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("ListEx")) return std::dynamic_pointer_cast<CListExUI>(shared_from_this());
    return CListUI::GetInterface(pstrName);
}

BOOL CListExUI::CheckColumEditable(int nColum)
{
    auto pHItem = std::dynamic_pointer_cast<CListContainerHeaderItemUI>(m_pHeader->GetItemAt(nColum));
...
std::shared_ptr<CRichEditUI> CListExUI::GetEditUI()
{
    if(!m_pEditUI)
    {
        m_pEditUI = std::make_shared<CRichEditUI>() ;
...
std::shared_ptr<CComboBoxUI> CListExUI::GetComboBoxUI()
{
    if(!m_pComboBoxUI)
    {
        m_pComboBoxUI = std::make_shared<CComboBoxUI>();
...
{
    std::shared_ptr<CControlUI> p = m_pHeader->GetItemAt(nColum);
    auto pHItem = std::dynamic_pointer_cast<CListContainerHeaderItemUI>(p->GetInterface(_T("ListContainerHeaderItem")));
...
void CListExUI::Notify(TNotifyUI& msg)
{
    ...
    if(msg.sType == _T("listheaditemchecked"))
    {
        ...
        for(int i = 0; i < pHeader->GetCount(); i++)
        {
            if(pHeader->GetItemAt(i) == msg.pSender)
            {
                ...
                {
                    std::shared_ptr<CControlUI> p = GetItemAt(j);
                    auto pLItem = std::dynamic_pointer_cast<CListTextExtElementUI>(p->GetInterface(_T("ListTextExElement")));
                    if(pLItem)
                    {
    ...
    if(strName == _T("ListEx_Edit") && m_pEditUI && m_nRow >= 0 && m_nColum >= 0)
    {
        ...
        else if(msg.sType == DUI_MSGTYPE_KILLFOCUS)
        {
            ...
            auto pRowCtrl = std::dynamic_pointer_cast<CListTextExtElementUI> (GetItemAt(m_nRow));
            if(pRowCtrl)
            {
    ...
    {
        int  iCurSel, m_nOldSel;
        iCurSel = (int) msg.wParam;
        m_nOldSel = (int) msg.lParam;
        ...
        else if(msg.sType == DUI_MSGTYPE_LISTITEMSELECT && m_nOldSel >= 0)
        {
            auto pRowCtrl = std::dynamic_pointer_cast<CListTextExtElementUI>(GetItemAt(m_nRow)) ;
            if(pRowCtrl)
            {
...
{
    std::shared_ptr<CControlUI> p = GetItemAt(nIndex);
    auto pLItem = std::dynamic_pointer_cast<CListTextExtElementUI>(p->GetInterface(_T("ListTextExElement")));
    if(pLItem)
    {
...
std::shared_ptr<CControlUI> CListContainerHeaderItemUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("ListContainerHeaderItem")) return std::dynamic_pointer_cast<CListContainerHeaderItemUI>(shared_from_this()); ;
...
void CListContainerHeaderItemUI::DoEvent(TEventUI& event)
{
    ...
    if(m_bCheckBoxable)
    {
        ...
        else if(event.Type == UIEVENT_BUTTONUP)
        {
            if((m_uCheckBoxState & UISTATE_CAPTURED) != 0)
            {
                ...
                {
                    SetCheck(!GetCheck());
                    auto pOwner = std::dynamic_pointer_cast<CContainerUI>(m_pParent);
                    if(pOwner)
                    {
                        m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_LISTHEADITEMCHECKED, pOwner->GetItemIndex(shared_from_this()), m_bChecked);
                    }
    ...
    if(event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK)
    {
        ...
        else
        {
            ...
            m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_LISTHEADERCLICK);
            Invalidate();
        }
        return;
    }
...
std::shared_ptr<CControlUI> CListTextExtElementUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("ListTextExElement")) return std::dynamic_pointer_cast<CListTextExtElementUI>(shared_from_this());
...
faw::string_t CListTextExtElementUI::GetText() const
{
...
void CListTextExtElementUI::SetText(faw::string_t pstrText)
{
    __super::SetText(pstrText);
}

void CListTextExtElementUI::SetOwner(std::shared_ptr<CControlUI> pOwner)
{
    CListElementUI::SetOwner(pOwner);
    m_pOwner = std::dynamic_pointer_cast<CListUI>(pOwner->GetInterface(_T("List")));
}
...
void CListTextExtElementUI::DoEvent(TEventUI& event)
{
    ...
    //检查是否需要显示编辑框或者组合框
    auto pListCtrl = std::dynamic_pointer_cast<CListExUI>(m_pOwner);
            ...
            else if(event.Type == UIEVENT_BUTTONUP)
            {
                if((m_uCheckBoxState & UISTATE_CAPTURED) != 0)
                {
                    if(::PtInRect(&rcCheckBox, event.ptMouse))
                    {
                        ...
                        if(m_pManager)
                        {
                            m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_LISTITEMCHECKED, MAKEWPARAM(GetIndex(), 0), m_bChecked);
                        }
                    }
                    m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
                    Invalidate();
...
bool CListTextExtElementUI::DoPaint(HDC hDC, const RECT& rcPaint, std::shared_ptr<CControlUI> pStopControl)
{
