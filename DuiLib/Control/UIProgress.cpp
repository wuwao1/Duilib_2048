...
std::shared_ptr<CControlUI> CProgressUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_PROGRESS) return std::dynamic_pointer_cast<CProgressUI>(shared_from_this());
...
void CProgressUI::PaintForeColor(HDC hDC)
{
    ...
    if(GetAdjustColor(m_dwForeColor)> 0x00FFFFFF)
