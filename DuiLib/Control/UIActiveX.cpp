...
public:
    CActiveXWnd() :
        m_iLayeredTick(0), m_bDrawCaret(false) {}
...
{
public:
    CActiveXFrameWnd(CActiveXUI* pOwner) :
        m_dwRef(1), m_pOwner(pOwner), m_pActiveObject(nullptr) {}
...
CActiveXCtrl::CActiveXCtrl() :
    m_pOwner(nullptr), m_pWindow(nullptr), m_pUnkSite(nullptr), m_pViewObject(nullptr), m_pInPlaceObject(nullptr)
{
    m_dwRef = 1;
...
{
    if(m_pOwner && m_pOwner->m_pOwner && m_pOwner->m_pOwner->GetManager() && m_pOwner->m_pOwner->GetManager()->IsLayered())
    {
...
LRESULT CActiveXWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        break;
    case WM_PRINT:
        lRes = OnPrint(uMsg, wParam, lParam);
        break;
    case WM_SETFOCUS:
    ...
    case WM_KILLFOCUS:
        ...
        break;
    case WM_ERASEBKGND:
        lRes = OnEraseBkgnd(uMsg, wParam, lParam);
        break;
    case WM_MOUSEACTIVATE:
        lRes = OnMouseActivate(uMsg, wParam, lParam);
        break;
    case WM_MOUSEWHEEL:
        break;
    default:
...
std::optional<LRESULT> CActiveXWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        ::SetTimer(m_hWnd, CARET_TIMER_ID, ::GetCaretBlinkTime(), nullptr);
    }
    return 0;
}
...
std::shared_ptr<CControlUI> CActiveXUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_ACTIVEX) return std::dynamic_pointer_cast<CActiveXUI>(shared_from_this());
...
bool CActiveXUI::DoPaint(HDC hDC, const RECT& rcPaint, std::shared_ptr<CControlUI> pStopControl)
{
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if(m_pManager) m_pManager->SendNotify((std::shared_ptr<CControlUI>) this, DUI_MSGTYPE_SHOWACTIVEX, 0, 0, false);
