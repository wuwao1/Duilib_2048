...
void CWebBrowserUI::NewWindow2(VARIANT_BOOL *&Cancel, BSTR bstrUrl)
{
    if(m_pWebBrowserEventHandler)
    {
...
bool CWebBrowserUI::CallJScript(const faw::string_t& strFunc, const faw::string_t& paramArray, VARIANT* pVarResult)
{
    ...
    CComBSTR bstr(paramArray.c_str());
...
std::shared_ptr<CControlUI> CWebBrowserUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_WEBBROWSER) return std::dynamic_pointer_cast<CWebBrowserUI>(shared_from_this());
