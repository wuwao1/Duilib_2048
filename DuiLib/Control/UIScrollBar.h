...
public:
    using ptr = std::shared_ptr<CScrollBarUI>;
    ...
    std::shared_ptr<CContainerUI> GetOwner() const ;
    void SetOwner(std::shared_ptr<CContainerUI> pOwner);
...
protected:
    ...
    int				m_nLineSize				= 8;
    std::shared_ptr<CContainerUI> m_pOwner  = nullptr;
