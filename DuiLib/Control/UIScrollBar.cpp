...
std::shared_ptr<CControlUI> CScrollBarUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_SCROLLBAR) return std::dynamic_pointer_cast<CScrollBarUI>(shared_from_this());
    return CControlUI::GetInterface(pstrName);
}

std::shared_ptr<CContainerUI> CScrollBarUI::GetOwner() const
{
    return m_pOwner;
}

void CScrollBarUI::SetOwner(std::shared_ptr<CContainerUI> pOwner)
{
    m_pOwner = pOwner;
}
...
void CScrollBarUI::SetHorizontal(bool bHorizontal)
{
    ...
    if(m_pOwner) m_pOwner->NeedUpdate();
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
    ...
    if(event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK)
    {
        ...
        else if(::PtInRect(&m_rcThumb, event.ptMouse))
        {
            ...
            m_pManager->SetTimer(shared_from_this(), SCROLL_TIMER_ID, 50U);
        }
        else
        {
            if(!m_bHorizontal)
        ...
        if(m_pManager) m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_SCROLL);
        return;
    }
    if(event.Type == UIEVENT_BUTTONUP)
    {
        m_nScrollRepeatDelay = 0;
        m_nLastScrollOffset = 0;
        m_pManager->KillTimer(shared_from_this(), SCROLL_TIMER_ID);
    ...
    if(event.Type == UIEVENT_TIMER && event.wParam == SCROLL_TIMER_ID)
    {
...
bool CScrollBarUI::DoPaint(HDC hDC, const RECT& rcPaint, std::shared_ptr<CControlUI> pStopControl)
{
    PaintBkColor(hDC);
    PaintBkImage(hDC);
    PaintBk(hDC);
