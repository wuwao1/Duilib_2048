...
std::shared_ptr<CControlUI> CTextUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_TEXT) return std::dynamic_pointer_cast<CTextUI>(shared_from_this());
...
void CTextUI::PaintText(HDC hDC)
{
    ...
    RECT rc = m_rcItem;
    auto rcTextPadding = GetTextPadding();
