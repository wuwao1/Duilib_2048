...
CHotKeyWnd::CHotKeyWnd(void) :
    m_pOwner(nullptr), m_hBkBrush(NULL), m_bInit(false)
{
}

void CHotKeyWnd::Init(CHotKeyUI::ptr pOwner)
{
    m_pOwner = pOwner;
    do
    {
        ...
        m_pOwner->SetText(GetHotKeyName()) ;
...
CHotKeyUI::CHotKeyUI() :
    m_pWindow(nullptr), m_wVirtualKeyCode(0), m_wModifiers(0), m_uButtonState(0)
{
...
std::shared_ptr<CControlUI> CHotKeyUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("HotKey")) return std::dynamic_pointer_cast<CHotKeyUI>(shared_from_this());
...
void CHotKeyUI::DoEvent(TEventUI& event)
{
    ...
    if(event.Type == UIEVENT_SETFOCUS && IsEnabled())
    {
        ...
        ASSERT(m_pWindow);
        m_pWindow->Init(std::dynamic_pointer_cast<CHotKeyUI>(shared_from_this()));
        Invalidate();
    }
