...
// 函数说明:
//************************************
CTreeNodeUI::CTreeNodeUI(std::shared_ptr<CTreeNodeUI> _ParentNode /*= nullptr*/)
{
    pHoriz = std::make_shared<CHorizontalLayoutUI> ();
    pFolderButton = std::make_shared<CCheckBoxUI> ();
    pDottedLine = std::make_shared<CLabelUI> ();
    pCheckBox = std::make_shared<CCheckBoxUI> ();
    pItemButton = std::make_shared<COptionUI> ();
...
// 函数说明:
//************************************
std::shared_ptr<CControlUI> CTreeNodeUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("TreeNode")) return std::dynamic_pointer_cast<CTreeNodeUI>(shared_from_this());
...
void CTreeNodeUI::DoEvent(TEventUI& event)
{
    ...
    if(event.Type == UIEVENT_DBLCLICK)
    {
        if(IsEnabled())
        {
            m_pManager->SendNotify(shared_from_this(), DUI_MSGTYPE_TREEITEMDBCLICK);
            Invalidate();
        }
        return;
    }
...
void CTreeNodeUI::Invalidate()
{
    ...
    if(GetParent())
    {
        auto pParentContainer = std::dynamic_pointer_cast<CContainerUI>(GetParent()->GetInterface(_T("Container")));
        if(pParentContainer)
        {
...
// 函数说明: 通过节点对象添加节点
//************************************
bool CTreeNodeUI::Add(std::shared_ptr<CControlUI> _pTreeNodeUI)
{
    auto pContainer = std::dynamic_pointer_cast<CTreeNodeUI>(_pTreeNodeUI->GetInterface(_T("TreeNode")));
    if(pContainer)
        return AddChildNode(pContainer);
...
// 函数说明:
//************************************
bool CTreeNodeUI::AddAt(std::shared_ptr<CControlUI> pControl, int iIndex)
{
    if(nullptr == std::dynamic_pointer_cast<CTreeNodeUI>(pControl->GetInterface(_T("TreeNode"))))
        return FALSE;
    if((size_t)iIndex>=mTreeNodes.size())
        return FALSE;

    std::shared_ptr<CTreeNodeUI> pIndexNode = std::dynamic_pointer_cast<CTreeNodeUI>(mTreeNodes[iIndex]);
    if(!pIndexNode)
    {
        mTreeNodes.push_back(pControl);
    }
    auto it = mTreeNodes.begin() + iIndex;
    mTreeNodes.insert(it, pControl); // 在位置iIndex处插入pControl


    if(!pIndexNode && pTreeView && pTreeView->GetItemAt(GetTreeIndex() + 1))
        pIndexNode = std::dynamic_pointer_cast<CTreeNodeUI>(pTreeView->GetItemAt(GetTreeIndex() + 1)->GetInterface(_T("TreeNode")));

    pControl = CalLocation(std::dynamic_pointer_cast<CTreeNodeUI>(pControl));

    if(pTreeView && pIndexNode)
        return pTreeView->AddAt(std::dynamic_pointer_cast<CTreeNodeUI>(pControl), pIndexNode);
    else
        return pTreeView->Add(std::dynamic_pointer_cast<CTreeNodeUI>(pControl));
...
// 函数说明:
//************************************
bool CTreeNodeUI::Remove(std::shared_ptr<CControlUI> pControl)
{
    return RemoveAt(std::dynamic_pointer_cast<CTreeNodeUI>(pControl));
}
...
{
    long level = 0;
    std::shared_ptr<CTreeNodeUI> pParentNode = GetParentNode();
...
// 函数说明:
//************************************
bool CTreeNodeUI::AddChildNode(std::shared_ptr<CTreeNodeUI> _pTreeNodeUI)
{
    if(!_pTreeNodeUI)
        return FALSE;

    if(nullptr == std::dynamic_pointer_cast<CTreeNodeUI>(_pTreeNodeUI->GetInterface(_T("TreeNode"))))
        return FALSE;
    ...
    if(pTreeView)
    {
        std::shared_ptr<CTreeNodeUI> pNode = std::dynamic_pointer_cast<CTreeNodeUI>(mTreeNodes[mTreeNodes.size() - 1]);
    ...
    if(nRet)
        mTreeNodes.push_back(_pTreeNodeUI);
...
// 函数说明:
//************************************
bool CTreeNodeUI::RemoveAt(std::shared_ptr<CTreeNodeUI> _pTreeNodeUI)
{
    auto it = std::find(mTreeNodes.begin(), mTreeNodes.end(), _pTreeNodeUI);
    if(it != mTreeNodes.end())
    {
        while(_pTreeNodeUI->IsHasChild())
            _pTreeNodeUI->RemoveAllNodes();
...
bool CTreeNodeUI::RemoveAllNodes()
{
    mTreeNodes.clear();
    return false;
}
...
// 函数说明:
//************************************
void CTreeNodeUI::SetParentNode(std::shared_ptr<CTreeNodeUI> _pParentTreeNode)
{
...
//************************************
// 函数名称: GetParentNode
// 返回类型: std::shared_ptr<CTreeNodeUI>
// 函数说明:
//************************************
std::shared_ptr<CTreeNodeUI> CTreeNodeUI::GetParentNode()
{
...
// 函数说明:
//************************************
void CTreeNodeUI::SetTreeView(std::shared_ptr<CTreeViewUI> _CTreeViewUI)
{
...
//************************************
// 函数名称: GetTreeView
// 返回类型: std::shared_ptr<CTreeViewUI>
// 函数说明:
//************************************
std::shared_ptr<CTreeViewUI> CTreeNodeUI::GetTreeView()
{
    return pTreeView;
}
...
// 函数说明:
//************************************
std::vector<std::shared_ptr<CControlUI>>& CTreeNodeUI::GetTreeNodes()
{
    return mTreeNodes;
}
...
// 函数说明:
//************************************
std::shared_ptr<CTreeNodeUI> CTreeNodeUI::GetChildNode(int _nIndex)
{
    return std::dynamic_pointer_cast<CTreeNodeUI>(mTreeNodes[_nIndex]);
}
...
int CTreeNodeUI::GetTreeIndex()
{
    ...
    {
        if(shared_from_this() == pTreeView->GetItemAt(nIndex))
            return nIndex;
    }
...
int CTreeNodeUI::GetNodeIndex()
{
    ...
    int index = -1;
    auto& nodes = GetParentNode()->GetTreeNodes();
    auto it = std::find(nodes.begin(), nodes.end(), shared_from_this());
    if(it != nodes.end())
        index = std::distance(nodes.begin(), it);
    return index;
}
...
// 函数说明:
//************************************
std::shared_ptr<CTreeNodeUI> CTreeNodeUI::GetLastNode()
{
    if(!IsHasChild()) return std::dynamic_pointer_cast<CTreeNodeUI>(shared_from_this());

    std::shared_ptr<CTreeNodeUI> nRetNode = nullptr;
    for(size_t nIndex = 0; nIndex < GetTreeNodes().size(); nIndex++)
    {
        std::shared_ptr<CTreeNodeUI> pNode = std::dynamic_pointer_cast<CTreeNodeUI>(GetTreeNodes()[nIndex]);
        if(!pNode) continue;
        if(pNode->IsHasChild())
...
// 函数说明: 缩进计算
//************************************
std::shared_ptr<CTreeNodeUI> CTreeNodeUI::CalLocation(std::shared_ptr<CTreeNodeUI> _pTreeNodeUI)
{
    _pTreeNodeUI->GetDottedLine()->SetVisible(TRUE);
    _pTreeNodeUI->GetDottedLine()->SetFixedWidth(pDottedLine->GetFixedWidth() + 16);
    _pTreeNodeUI->SetParentNode(std::dynamic_pointer_cast<CTreeNodeUI>(shared_from_this()));
...
CTreeViewUI::CTreeViewUI(void) :
    m_bVisibleFolderBtn(TRUE), m_bVisibleCheckBtn(FALSE), m_uItemMinWidth(0)
{
...
// 函数说明:
//************************************
std::shared_ptr<CControlUI>  CTreeViewUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == _T("TreeView")) return shared_from_this();
...
// 函数说明:
//************************************
bool CTreeViewUI::Add(std::shared_ptr<CTreeNodeUI> pControl)
{
    if(!pControl) return false;
    if(nullptr == std::dynamic_pointer_cast<CTreeNodeUI>(pControl->GetInterface(_T("TreeNode")))) return false;
    ...
    if(pControl->GetCountChild() > 0)
    {
        ...
        {
            std::shared_ptr<CTreeNodeUI> pNode = pControl->GetChildNode(nIndex);
            if(pNode) Add(pNode);
        }
    }

    pControl->SetTreeView(std::dynamic_pointer_cast<CTreeViewUI>(shared_from_this()));
    return true;
}
...
// 参数信息: CTreeNodeUI * pControl
// 参数信息: int iIndex
// 函数说明: 该方法不会将待插入的节点进行缩位处理，若打算插入的节点为非根节点，请使用AddAt(std::shared_ptr<CTreeNodeUI> pControl,std::shared_ptr<CTreeNodeUI> _IndexNode) 方法
//************************************
long CTreeViewUI::AddAt(std::shared_ptr<CTreeNodeUI> pControl, int iIndex)
{
    if(!pControl) return -1;
    if(nullptr == std::dynamic_pointer_cast<CTreeNodeUI>(pControl->GetInterface(_T("TreeNode")))) return -1;
    pControl->OnNotify += MakeDelegate(this, &CTreeViewUI::OnDBClickItem);
    pControl->GetFolderButton()->OnNotify += MakeDelegate(this, &CTreeViewUI::OnFolderChanged);
    pControl->GetCheckBox()->OnNotify += MakeDelegate(this, &CTreeViewUI::OnCheckBoxChanged);
    pControl->SetVisibleFolderBtn(m_bVisibleFolderBtn);
...
// 函数说明:
//************************************
bool CTreeViewUI::AddAt(std::shared_ptr<CTreeNodeUI> pControl, std::shared_ptr<CTreeNodeUI> _IndexNode)
{
...
//************************************
bool CTreeViewUI::Remove(std::shared_ptr<CTreeNodeUI> pControl)
{
    if(pControl->GetCountChild() > 0)
    {
        int nCount = pControl->GetCountChild();
...
{
    std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(GetItemAt(iIndex)) ;
    Remove(pItem);
    return TRUE;
}
...
bool CTreeViewUI::OnCheckBoxChanged(void* param)
{
    ...
    {
        auto pCheckBox = std::dynamic_pointer_cast<CCheckBoxUI>(pMsg->pSender) ;
        std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(pCheckBox->GetParent()->GetParent());
...
bool CTreeViewUI::OnFolderChanged(void* param)
{
    ...
    if(pMsg->sType == DUI_MSGTYPE_SELECTCHANGED)
    {
        auto pFolder = std::dynamic_pointer_cast<CCheckBoxUI>(pMsg->pSender);
        std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(pFolder->GetParent()->GetParent());
...
bool CTreeViewUI::OnDBClickItem(void* param)
{
    ...
    {
        std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(pMsg->pSender);
        std::shared_ptr<CCheckBoxUI> pFolder = pItem->GetFolderButton();
...
// 函数说明:
//************************************
bool CTreeViewUI::SetItemCheckBox(bool _Selected, std::shared_ptr<CTreeNodeUI> _TreeNode /*= nullptr*/)
{
    if(_TreeNode)
    {
        if(_TreeNode->GetCountChild() > 0)
        {
            ...
            for(int nIndex = 0; nIndex < nCount; nIndex++)
            {
                std::shared_ptr<CTreeNodeUI> pItem = _TreeNode->GetChildNode(nIndex);
    ...
    else
    {
        ...
        while(nIndex < nCount)
        {
            std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(GetItemAt(nIndex));
            pItem->GetCheckBox()->Selected(_Selected);
            if(pItem->GetCountChild())
                SetItemCheckBox(_Selected, pItem);
...
// 函数说明:
//************************************
void CTreeViewUI::SetItemExpand(bool _Expanded, std::shared_ptr<CTreeNodeUI> _TreeNode /*= nullptr*/)
{
    if(_TreeNode)
    {
        if(_TreeNode->GetCountChild() > 0)
...
void CTreeViewUI::SetVisibleFolderBtn(bool _IsVisibled)
{
    ...
    for(int nIndex = 0; nIndex < nCount; nIndex++)
    {
        std::shared_ptr<CTreeNodeUI> pItem = std::dynamic_pointer_cast<CTreeNodeUI>(this->GetItemAt(nIndex));
...
void CTreeViewUI::SetItemMinWidth(UINT _ItemMinWidth)
{
    ...
    for(int nIndex = 0; nIndex < GetCount(); nIndex++)
    {
        std::shared_ptr<CTreeNodeUI> pTreeNode = std::dynamic_pointer_cast<CTreeNodeUI>(GetItemAt(nIndex));
        if(pTreeNode)
        {
