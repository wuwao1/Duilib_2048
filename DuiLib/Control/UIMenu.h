...
public:
    using ptr = std::shared_ptr<CMenuUI>;
    CMenuUI();
    virtual ~CMenuUI();
    ...
    virtual bool Add(std::shared_ptr<CControlUI> pControl) override;
    virtual bool AddAt(std::shared_ptr<CControlUI> pControl, int iIndex) override;

    virtual int GetItemIndex(std::shared_ptr<CControlUI> pControl) const override;
    virtual bool SetItemIndex(std::shared_ptr<CControlUI> pControl, int iIndex) override;
    virtual bool Remove(std::shared_ptr<CControlUI> pControl) override;
...
public:
    ...
    static CMenuWnd* CreateMenu(std::shared_ptr<CMenuElementUI> pOwner, std::variant<UINT, faw::string_t> xml, POINT point,
...
public:
    using ptr = std::shared_ptr<CMenuWnd>;
    CMenuWnd();
    virtual ~CMenuWnd();
    ...
    void Init(std::shared_ptr<CMenuElementUI>, std::variant<UINT, faw::string_t> xml, POINT point,
              CPaintManagerUI* pMainPaintManager, CStdStringPtrMap* pMenuCheckInfo = nullptr,
              DWORD dwAlignment = eMenuAlignment_Left | eMenuAlignment_Top);
    LPCTSTR GetWindowClassName() const;
    void OnFinalMessage(HWND hWnd);
    void Notify(TNotifyUI& msg);
    std::shared_ptr<CControlUI> CreateControl(faw::string_t pstrClassName);
    ...
    // 获取根菜单控件，用于动态添加子菜单
    std::shared_ptr<CMenuUI> GetMenuUI();
...
public:
    ...
    std::shared_ptr<CMenuElementUI> m_pOwner;
    std::shared_ptr<CMenuUI>	m_pLayout;
...
class UILIB_API CMenuElementUI: public CListContainerElementUI
{
    ...
    friend CMenuWnd;
public:
    using ptr = std::shared_ptr<CMenuElementUI>;
    CMenuElementUI();
    virtual ~CMenuElementUI();

    faw::string_t GetClass() const override;
    std::shared_ptr<CControlUI> GetInterface(faw::string_t pstrName) override;
    bool DoPaint(HDC hDC, const RECT& rcPaint, std::shared_ptr<CControlUI> pStopControl) override;
