...
typedef struct
{
    faw::string_t name;       // filename within the zip
    std::shared_ptr<unsigned char[]> dataPtr;
    long dataSize;              // may be -1 if not yet known (e.g. being streamed in)
} CacheZipEntry;


HZIP OpenZip(const TCHAR *fn, const char *password = nullptr);
HZIP OpenZip(void *z, unsigned int len, const char *password = nullptr);
