...
class CPSD
{
    struct HEADER_INFO
    {
        ...
        //								They are full unsigned 16朾it values as in Apple抯 RGBColor data
        //								structure. Pure red=65535,0,0.
        //		4		CMYK			The four values in the color data are cyan, magenta, yellow, and
        //								black. They are full unsigned 16朾it values. 0=100% ink. Pure
        ...
        //								and b chrominance.
        //								Lightness is a 16朾it value from 0...100. The chromanance components
        //								are each 16朾it values from ?28...127. Gray values
    ...
    struct DISPLAY_INFO
    {
        ...
        //							brightness. They are full unsigned 16朾it values as in Apple抯
        ...
        //							and b chrominance.
        //							Lightness is a 16朾it value from 0...10000. The chromanance components
        //							are each 16朾it values from ?2800...12700. Gray values
        //							are represented by chrominance components of 0. Pure
        //							white=10000,0,0.
...
public:
    ...
    bool ThumbNailIncluded() const
    {
        return mbThumbNailFilled;
    }
    void DPI(int &x, int &y) const
    {
        x = resolution_info.hRes;
        y = resolution_info.vRes;
    }
    void Dimensions(int &cx, int &cy) const
    {
        cx = header_info.nWidth;
        cy = header_info.nHeight;
    }
    int BitsPerPixel() const
    {
        return header_info.nBitsPerPixel;
    }
    int GlobalAngle() const
    {
        return mnGlobalAngle;
    }
...
void CPSD::ProccessBuffer(unsigned char* pData)
{
    ...
    case 4:	// CMYK
    {
        ...
        for(int i = 0; i < nPixels; i++, pSrc += header_info.nChannels, pDst += bAlpha ? 4 : 3)
        {
            ...
            if(0 > nRed) nRed = 0;
            else if(255 < nRed) nRed = 255;
            if(0 > nGreen) nGreen = 0;
            else if(255 < nGreen) nGreen = 255;
            if(0 > nBlue) nBlue = 0;
            else if(255 < nBlue) nBlue = 255;
