    ...
    RGBQUAD* const rplColor)
{
    ...
    if(x<0) xi--;    //these replace (incredibly slow) floor (Visual c++ 2003, AMD Athlon)
    ...
    default:
    {
        ...
        if(xi<-1 || xi>=head.biWidth || yi<-1 || yi>=head.biHeight)     //all 4 points are outside bounds?:
        {
            ...
            default:
                ...
                OverflowCoordinates(x,y,ofMethod);
                xi=(int32_t)(x);
                if(x<0) xi--;    //x and/or y have changed ... recalculate xi and yi
                yi=(int32_t)(y);
                if(y<0) yi--;
            }//switch
        }//if
...
float CxImage::KernelBSpline(const float x)
{
    ...
    if((xp2) <= 0.0f) a = 0.0f;
    else a = xp2*xp2*xp2; // Only float, not float -> double -> float
    if((xp1) <= 0.0f) b = 0.0f;
    else b = xp1*xp1*xp1;
    if(x <= 0) c = 0.0f;
    else c = x*x*x;
