...
std::shared_ptr<CControlUI> CTileLayoutUI::GetInterface(faw::string_t pstrName)
{
    if(pstrName == DUI_CTRL_TILELAYOUT) return std::dynamic_pointer_cast<CTileLayoutUI>(shared_from_this());
...
void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    for(size_t it1 = 0; it1 < m_items.size(); it1++)
    {
        std::shared_ptr<CControlUI> pControl = GetItemAt(it1);
        ...
        {
            int iIndex = iCount;
            for(size_t it2 = it1; it2 < m_items.size(); it2++)
            {
                std::shared_ptr<CControlUI> pLineControl = GetItemAt(it2);
